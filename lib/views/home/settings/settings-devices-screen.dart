...
class DeviceViewState extends State<DevicesScreen> with Lifecycle<DevicesScreen> {
  ...
  void onDismissDeviceOptions() {
    setState(() {
      selectedDevices = null;
    });
  }

  void onToggleAllDevices({required List<Device> devices}) {
  ...
  void onToggleModifyDevice({required Device device}) {
  ...
  Future<void> onDeleteDevices(BuildContext context, List<Device> devices, _Props props) async {
    final store = StoreProvider.of<AppState>(context);
    ...
    return AppBar(
      ...
      actions: <Widget>[
        IconButton(
          ...
          color: Colors.white,
          onPressed: selectedDevices!.length != 1 ? null : () => props.onRenameDevice(context, selectedDevices![0]),
        ),
        IconButton(
          ...
          color: Colors.white,
          onPressed: selfSelectedDevice != -1 ? null : () => onDeleteDevices(context, selectedDevices ?? [], props),
        ),
        IconButton(
